"""RFC-001 v1.0.0 tool input/output JSON Schemas — public wire surface.

Promoted from ``llm_kernel._rfc_schemas`` (private) to
``llm_kernel.wire.tools`` (public) in S5.0.3a.  All content is
identical to the original; the move makes the contract surface
importable by external drivers without depending on underscore-prefixed
internals.
...
def validate_tool_output(tool_name: str, result: Dict[str, Any]) -> Optional[str]:
    """Validate ``result`` against the RFC-001 output schema for ``tool_name``.

    Symmetric with :func:`validate_tool_input` but checks the output
    schema.  Returns ``None`` on success, error string on failure.
    Unknown tools return ``None`` (no output schema to check).
    """
    entry = TOOL_CATALOG.get(tool_name)
    if entry is None:
        return None
    _input_schema, output_schema, _description = entry
    ...
    try:
        ...
        return _hand_validate(output_schema, result)
...
K3G_OPERATOR_ACCEPTED_INJECTION_PERSISTED: str = "K3G"
...
__all__ = [
    # JSON schema helpers (private underscore names are module-private only)
    "JSONSchema",
    "TOOL_CATALOG",
    "NATIVE_TOOLS",
    "PROXIED_TOOLS",
